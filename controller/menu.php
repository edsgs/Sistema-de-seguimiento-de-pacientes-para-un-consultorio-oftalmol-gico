<?php
  ...
  $menu = new Menu(); //Hace referencia a la clase Menu de la carpeta "models"
  ...
  switch($_GET["op"]){
    ...
    case "activarydesactivar": //eliminar
        $datos = $menu->get_menu_x_id($_POST["men_id"]); //Se obtiene el id del elemento que se desea eliminar
        if(is_array($datos) == true and count($datos) > 0){ //Si es un array y lo que me esta capturando es mayor a 0
    ...
    case "guardaryeditar":
        if(empty($_POST["men_id"])){ //Si "men_id" viene vacio significa que voy a realizar un registro nuevo
